...
6. The FE GPU cooler copes with compute workloads surprisingly well; temp stays around 75C, and we never saw the GPU break 77C. With that said, the FE card _will not stack_ and is definitely an exception to the "always buy reference design" rule. Only buy the FE card if it will be the only card in the system, while also keeping in mind this style cooler (vertical-fin heatsink with axial fans) will exhaust hot air _into_ the chassis instead of out of the chassis. Blower-style OEM GPUs are strongly recommended in this instance.
